...
- [x] M4 PPO linear readout, plan: `docs/plans/2026-09-10-m4-ppo.md`
  ...
  - [x] train, greedy evaluation against the M3 baselines
    ...
    - Caveats: the greedy evaluation's 8 worlds are the 8 the training run started on (same key), so it is not held out, while the two baselines never trained on them; the trained readout had not been run against black/static/disconnected at this point, so nothing separated the frozen brain's DN rates from any noisy interoceptive feature vector, and the greedy policy is a near-perfect forward/do alternation that needs no vision. See the control run below
    - control run, `scripts/eval_ppo_controls.py`, `make controls`: the same frozen readout replayed greedily on a held-out `PRNGKey(1)`, 8 envs, 2,000-action cap, six conditions in 78.1 min, into `outputs/ppo_controls.json` and `outputs/ppo_controls.png`. Mean survival (per-env min-max) ppo/greedy 176.4 (83-244), ppo/greedy/black 204.4 (47-459), ppo/greedy/static 143.8 (54-252), ppo/greedy/disconnected 159.4 (70-262), alternate/full 192.9 (74-317), full/random 149.9 (59-238); achievement totals 25, 27, 22, 12, 10, 23; dominant actions `forward` 0.48 with `do` 0.46 for greedy, black and static alike, `do` 1.00 for disconnected, an exact `forward` 0.50 / `do` 0.50 for the open-loop alternation and a flat 0.14-0.15 for random. Blacking vision out or freezing it leaves the trained policy's action mix and score essentially unchanged (27 and 22 achievements against 25), so the readout is not reading vision at all; cutting every drive collapses it to a constant `do` and halves the achievements to 12, so it does depend on the brain's state, but only enough to break the forward/do tie; and against the open-loop alternation it survives less (176.4 versus 192.9) while scoring 25 achievements against 10, though 8 of the 25 are `wake_up`, self-generated by the 4% sleep action the clock never takes, so the like-for-like gap is 17 versus 10 with random at 15 yet barely clears random's 23 on the same fresh key -- so on this one seed the trained readout is best described as a clock with a small drive-dependent jitter, not a visually guided policy
    - Reproducibility: `outputs/` is gitignored, so `ppo_params.npz` and every number in the M4 bullets come from one run on the pod. The final-review fix that dropped a dead key rebind in `ppo.update` changed `split(key, 4)` to `split(key, 3)`, and JAX's split is not prefix-stable, so `make train` at the current commit draws a different key stream and will not reproduce these parameters or numbers; only the qualitative comparison should survive a retrain
